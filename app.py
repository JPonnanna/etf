...
def fetch_snapshot():
    ...
    for t in gold_etf_tickers:
        try:
            ...
            disc = None
            if ltp is not None and inav:
                disc = (ltp - inav) / inav * 100
            rows.append({
                "Ticker": t,
                "LTP": ltp,
                "iNAV ok?": inav_ok,
            ...
            rows.append({"Ticker": t, "LTP": None, "iNAV ok?": False, "iNAV": None, "Disc/Prem %": None, "Error": str(e)})
    return pd.DataFrame(rows)

# simple page auto-refresh every 120s (works on Streamlit Cloud)
st.markdown('<meta http-equiv="refresh" content="120">', unsafe_allow_html=True)

st.title("Gold ETFs — LTP vs iNAV (NSE)")
st.caption(f"Updated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} (auto refresh ~120s)")
